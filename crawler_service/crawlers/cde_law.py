...
class CDELawCrawler(BaseCrawler):
    """采集 CDE 网站法律法规栏目."""
    ...
    start_urls = ["https://www.cde.org.cn/main/policy/listpage/9f9c74c73e0f8f56a8bfbc646055026d"]
    ...
    def crawl(self) -> List[CrawlResult]:
        ...
        for entry in selected:
            try:
                ...
                self.logger.warning("解析法律法规失败 url=%s err=%s", entry["url"], exc)
        self.logger.info("法律法规采集 %s 条记录", len(results))
        return results

    def _fetch_listing_html(self) -> str:
        """使用 Playwright 渲染列表页，绕过反爬机制."""
        try:
            self.logger.info("使用 Playwright 渲染法律法规列表页: %s", self.list_url)
            html = fetch_html(
                self.list_url,
    ...
    def _extract_entries(self, html: str) -> List[Dict[str, str]]:
        ...
        if entries:
            self.logger.info("从旧版结构解析出 %d 条法律法规", len(entries))
        else:
            self.logger.warning("未能从任何结构解析出法律法规条目")
        return entries
...
registry.register(CDELawCrawler)

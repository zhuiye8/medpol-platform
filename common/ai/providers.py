...
class AIProviderFactory:
    """根据 .env 配置创建 OpenAI/DeepSeek 客户端"""

    def __init__(self) -> None:
        self.settings = get_settings()

    def get_client(self) -> ProviderClient:
        ...
        for provider in providers:
            ...
            except AIProviderError as exc:  # pragma: no cover - 配置错误
    ...
    def _build_client(self, provider: str) -> ProviderClient:
        ...
        model = self.settings.ai_chat_model or self.settings.openai_model
        client = OpenAI(api_key=api_key, base_url=base_url)
        return ProviderClient(client=client, model=model)
